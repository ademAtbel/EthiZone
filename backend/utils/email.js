...
const getTransporter = (user, pass, host, port) => {
  ...
  const targetHost = isGmail ? 'smtp.gmail.com' : (host || 'smtp.gmail.com');
  const targetPort = parseInt(port) || 465;
...
const sendOtpEmail = async (email, code) => {
  ...
  const mailText = `Your one-time login code is: ${code}. This code is valid for 90 seconds.`;
  const mailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; background-color: #ffffff;">
      ...
      <p style="font-size: 14px; color: #e53e3e; font-weight: bold;">Warning: This code is valid for 90 seconds.</p>
