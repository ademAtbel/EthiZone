import React, { createContext, useContext, useState, useEffect } from 'react';

const AppContext = createContext();

const translations = {
  en: {
    // Navbar
    stores: 'Stores',
    handymen: 'Hire Me',
    services: 'Services',
    organizations: 'Jobs',
    real_estate: 'Houses',
    automotive: 'Automotive',
    used_items: 'Used Items',
    be_partner: 'Be Our Partner',
    login: 'Login',
    logout: 'Logout',
    dashboard: 'Dashboard',
    admin_panel: 'Admin Panel',
    qr_code: '🖨️ QR Code',
    my_web: '🔗 My Web',
    
    // Home Page
    hero_title: 'Direct-Connect Local Directory',
    hero_desc: 'Locate local stores, pharmacy items, professional services, tax filing experts, and handymen near you. Contact them instantly via phone or SMS with zero middle-man fees.',
    search_placeholder: 'What are you looking for? (e.g. prescription, carpenter, jacket...)',
    search_btn: 'Search',
    all_categories: 'All Categories',
    all_listings: 'All Marketplace Listings',
    active_postings_for: 'Active Postings for "{type}"',
    contact_for_price: 'Contact for Price',
    salary_offered: 'Salary Offered',
    price: 'Price',
    visit_store: 'Visit Store',
    call: 'Call',
    sms: 'SMS',
    verified: '✓ Verified',
    no_listings_found: 'No listings match your selection.',
    
    // Dashboard page
    post_marketplace: 'Post to EthiZone',
    listing_title: 'Listing Title',
    listing_desc: 'Listing Description',
    price_val: 'Price / Value Offered ($)',
    publish_btn: '+ Post to EthiZone',
    visit_storefront_btn: '🔗 Visit My Storefront',
    online_status: 'Online Customer Line Status',
    status_online: '🟢 Online',
    status_offline: '🔴 Offline (Vacation Mode)',
    manage_nav: 'Customize Storefront Navigation',
    nav_help: 'Add up to 5 custom website references (e.g. Yelp, About Us, Booking portal) directly onto your storefront\'s navigation bar.',
    link_label: 'LINK LABEL',
    dest_url: 'DESTINATION URL',
    add_link: '+ Add Nav Link',
    my_listings: 'My Marketplace Postings',
    references_feedback: 'Provider Reference Log & Feedback',
    no_listings_yet: 'You haven\'t posted any items yet.',
    no_refs_yet: 'No customer references logged yet.',
    verified_reviews_desc: 'Below are authenticated client records & star reviews left by visitors who completed service transactions with your business.',
    
    // Static Pages (User can update these later)
    about_us_content: 'EthiZone is a multi-category marketplace designed to connect people who want to buy, sell, rent, offer services, find work, hire talent, and grow businesses through one simple platform. A person may buy today and sell tomorrow. EthiZone brings these opportunities into one organized platform.',
    contact_tab_content: 'We are here to help users, sellers, service providers, store owners, professionals, and customers use EthiZone safely and effectively. Support Email: support@ethizone.com | Business and Partnership Email: partners@ethizone.com | Safety and Report Email: safety@ethizone.com. Response Time: We aim to respond as soon as possible.',
    privacy_policy_content: 'EthiZone respects user privacy and is committed to protecting personal information. This Privacy Policy explains how information may be collected and used when users access EthiZone, create an account, register a business or profile, publish listings, communicate with others, or use marketplace services.',
    terms_of_service_content: 'By accessing or using EthiZone, users agree to follow these Terms of Service. If a user does not agree, they should not use the platform. EthiZone is a digital marketplace and technology platform. Unless EthiZone clearly provides a separate written service or guarantee, EthiZone is not the seller, buyer, broker, landlord, tenant, employer, employee, delivery provider, payment guarantor, legal advisor, or agent in user transactions.',
    legal_disclaimer_title: '⚠️ Important Legal Disclaimer',
    legal_disclaimer: 'For any money transaction, EthiZone is NOT responsible. The seller is solely responsible for their products and services.',
    
    // Storefront page
    catalog_listings: 'Catalog & Active Listings',
    no_posts_yet: 'No products, services, or job openings posted by this business yet.',
    no_match_search: 'No listings matched your store search query.',
    search_in_store: '🔍 Search items in this store...',
    cust_references: 'Customer References',
    no_reviews_yet: 'No references posted yet. Be the first to leave feedback!',
    direct_line: 'Direct Line',
    submit_ref: 'Submit Reference',
    rating_score: 'Rating Score (1-5)',
    comment: 'Comment / Review Message',
    submit_btn: 'Submit Reference',
    
    // Login / Register page
    create_account: 'Create Professional Account',
    register_desc: 'Register to list products, advertise skills, and connect directly with local clients.',
    username: 'Username / Account Handle',
    email: 'Email Address',
    phone: 'Phone Number (Direct Client Line)',
    password: 'Password',
    account_role: 'Account Role / Purpose',
    role_individual: 'Individual Seller (used items, personal)',
    role_handyman: 'Handyman / Home Provider',
    role_business: 'Local Business / Agency Store',
    business_type: 'Business Category Type',
    select_type: 'Select Business Type',
    store: 'Store / Retail Shop',
    service: 'Service / Consultant',
    organization: 'Organization / Company',
    real_estate_type: 'Real Estate / Housing Broker',
    automotive_type: 'Automotive / Car Dealer',
    register_btn: 'Register & Initialize',
    already_account: 'Already have an account? Login here',
    login_account: 'Login to Account',
    no_account: 'New to the platform? Register here',

    // Registration page specific (4-color unified theme)
    reg_title: 'Create Account',
    reg_subtitle: 'Join the direct-connect local marketplace',
    reg_fullname: 'Full Name',
    reg_email: 'Email Address',
    reg_phone: 'Phone Number (For direct customer calls)',
    reg_password: 'Password',
    reg_account_type: 'Account Type',
    reg_individual: 'Individual',
    reg_sell_personal: 'Sell personal items',
    reg_handyman: 'Handyman',
    reg_list_skills: 'List skills',
    reg_business: 'Business',
    reg_stores_agencies: 'Stores & agencies',
    reg_business_type: 'Business Type',
    reg_store: 'Store',
    reg_sells_products: 'Sells Products',
    reg_service: 'Service',
    reg_offers_services: 'Offers Services',
    reg_organization: 'Organization',
    reg_hiring_jobs: 'Hiring & Jobs',
    reg_real_estate: 'Real Estate',
    reg_housing_listings: 'Housing Listings',
    reg_automotive: 'Automotive',
    reg_car_listings: 'Car Listings',
    reg_category_type: 'Business Category Type',
    reg_btn: 'Register Now',
    reg_creating: 'Creating Account...',
    reg_login_here: 'Login Here',
    reg_already_have: 'Already have an account?',
    reg_event: 'Events',
    reg_posts_events: 'Host Events',

    // Category names
    boutique: 'Boutique',
    pharmacy: 'Pharmacy',
    liquor_store: 'Liquor Store',
    grocery_store: 'Grocery Store',
    entertainment: 'Entertainment',
    arts_slots_culture: 'Arts & Culture',
    religious: 'Religious',
    social: 'Social',
    educational: 'Educational',
    sports: 'Sports',
    charity: 'Charity',
    electronics_shop: 'Electronics Shop',
    bookstore: 'Bookstore',
    furniture: 'Furniture',
    hardware_store: 'Hardware Store',
    cafe_slots_restaurant: 'Cafe & Restaurant',
    cafe_restaurant: 'Cafe & Restaurant',
    jewelry_slots_accessories: 'Jewelry & Accessories',
    jewelry_accessories: 'Jewelry & Accessories',
    gift_slots_toy_shop: 'Gift & Toy Shop',
    gift_toy: 'Gift & Toy Shop',
    other_store: 'Other Store',
    law_office: 'Law Office',
    tax_office: 'Tax Office',
    dental_clinic: 'Dental Clinic',
    clinic: 'Clinic',
    consulting_firm: 'Consulting Firm',
    cleaning_agency: 'Cleaning Agency',
    beauty_salon: 'Beauty Salon',
    tech_corporation: 'Tech Corporation',
    construction_company: 'Construction Company',
    healthcare_group: 'Healthcare Group',
    educational_institution: 'Educational Institution',
    non_profit_org: 'Non-Profit Org',
    other: 'Other',
    residential_homes: 'Residential Homes',
    rental_apartments: 'Rental Apartments',
    commercial_real_estate: 'Commercial Real Estate',
    land_slots: 'Land & Lots',
    used_car_dealership: 'Used Car Dealership',
    car_rental_service: 'Car Rental Service',
    auto_repair_workshop: 'Auto Repair Workshop',
    spare_parts_dealer: 'Spare Parts Dealer',
    
    // Grocery categories
    grocery_home: 'Home',
    grocery_all_products: 'All Products',
    grocery_fruits_vegetables: 'Fruits & Vegetables',
    grocery_meat_fish: 'Meat & Fish',
    grocery_dairy_eggs: 'Dairy & Eggs',
    grocery_bakery: 'Bakery',
    grocery_pantry: 'Pantry',
    grocery_drinks: 'Drinks',
    grocery_household: 'Household',
    grocery_baby_personal_care: 'Baby & Personal Care',
    grocery_new_arrivals: 'New Arrivals',
    grocery_on_sale: 'On Sale',
    grocery_about_us: 'About Us',
    
    // Liquor categories
    liquor_home: 'Home',
    liquor_all_drinks: 'All Drinks',
    liquor_wine: 'Wine',
    liquor_beer: 'Beer',
    liquor_whiskey: 'Whiskey',
    liquor_vodka: 'Vodka',
    liquor_gin: 'Gin',
    liquor_rum: 'Rum',
    liquor_tequila: 'Tequila',
    liquor_champagne: 'Champagne',
    liquor_mixers: 'Mixers',
    liquor_snacks: 'Snacks',
    liquor_gift_sets: 'Gift Sets',
    liquor_new_arrivals: 'New Arrivals',
    liquor_on_sale: 'On Sale',
    liquor_about_us: 'About Us',

    // Electronics categories
    electronics_home: 'Home',
    electronics_all_electronics: 'All Electronics',
    electronics_phones: 'Phones',
    electronics_laptops: 'Laptops',
    electronics_tv_audio: 'TV & Audio',
    electronics_gaming: 'Gaming',
    electronics_accessories: 'Accessories',
    electronics_smart_devices: 'Smart Devices',
    electronics_appliances: 'Appliances',
    electronics_new_arrivals: 'New Arrivals',
    electronics_on_sale: 'On Sale',
    electronics_refurbished: 'Refurbished',
    electronics_warranty: 'Warranty',
    electronics_about_us: 'About Us',

    // Law Office categories
    law_home: 'Home',
    law_about_us: 'About Us',
    law_all_services: 'Legal Services',
    law_family_law: 'Family Law',
    law_business_law: 'Business Law',
    law_immigration: 'Immigration',
    law_real_estate_law: 'Real Estate Law',
    law_contract_review: 'Contract Review',
    law_consultation: 'Consultation',
    law_attorneys: 'Attorneys',
    law_book_appointment: 'Book Appointment',
    law_contact: 'Contact',
    schedule_appointment: 'Book an Appointment',

    // Tax Office categories
    tax_home: 'Home',
    tax_about_us: 'About Us',
    tax_all_services: 'Tax Services',
    tax_personal_tax: 'Personal Tax',
    tax_business_tax: 'Business Tax',
    tax_vat: 'VAT',
    tax_payroll_tax: 'Payroll Tax',
    tax_accounting: 'Accounting',
    tax_bookkeeping: 'Bookkeeping',
    tax_consultation: 'Tax Consultation',
    tax_book_appointment: 'Book Appointment',
    tax_contact: 'Contact',

    // Clinic categories
    clinic_home: 'Home',
    clinic_about_us: 'About Us',
    clinic_all_services: 'Services',
    clinic_doctors: 'Doctors',
    clinic_general_checkup: 'General Checkup',
    clinic_laboratory: 'Laboratory',
    clinic_pharmacy: 'Pharmacy',
    clinic_dental: 'Dental',
    clinic_eye_care: 'Eye Care',
    clinic_womens_health: "Women's Health",
    clinic_book_appointment: 'Book Appointment',
    clinic_contact: 'Contact',

    // Cleaning categories
    cleaning_home: 'Home',
    cleaning_about_us: 'About Us',
    cleaning_all_services: 'Services',
    cleaning_house_cleaning: 'House Cleaning',
    cleaning_office_cleaning: 'Office Cleaning',
    cleaning_deep_cleaning: 'Deep Cleaning',
    cleaning_move_in_out: 'Move-In / Move-Out',
    cleaning_carpet_cleaning: 'Carpet Cleaning',
    cleaning_disinfection: 'Disinfection',
    cleaning_book_now: 'Book Now',
    cleaning_contact: 'Contact',

    // Beauty Salon categories
    beauty_home: 'Home',
    beauty_about_us: 'About Us',
    beauty_all_services: 'Services',
    beauty_hair: 'Hair',
    beauty_makeup: 'Makeup',
    beauty_nails: 'Nails',
    beauty_facial: 'Facial',
    beauty_barber: 'Barber',
    beauty_spa: 'Spa',
    beauty_packages: 'Packages',
    beauty_gallery: 'Gallery',
    beauty_book_appointment: 'Book Appointment',
    beauty_contact: 'Contact',

    // Login page specific
    login_title_pw: 'Welcome Back',
    login_title_otp: 'Verification Login',
    login_subtitle_pw: 'Login to access your dashboard and listings',
    login_subtitle_otp: 'Enter your email to receive a 6-digit login code',
    login_email: 'Email Address',
    login_password: 'Password',
    login_forgot: 'Forgot Password? Log in with One-Time Code',
    login_btn_loading: 'Logging in...',
    login_btn_pw: 'Sign In',
    login_otp_label: 'One-Time Code (6-Digits)',
    login_expires_in: 'Code expires in',
    login_seconds: 'seconds',
    login_expired: 'Code expired.',
    login_resend: 'Resend Code',
    login_processing: 'Processing...',
    login_verify_signin: 'Verify & Sign In',
    login_send_otp: 'Send One-Time Code',
    login_back_standard: 'Back to Standard Password Login',
    login_no_account: "Don't have an account?",
    login_register_here: 'Register Here',

    // Homepage Landing specific
    landing_hero_title: 'Everything You Need in One Marketplace',
    landing_hero_desc: "Shop products, discover stores, find services, browse houses, cars, jobs, and more on the world's most versatile ethical marketplace.",
    landing_start_shopping: 'Start Shopping',
    landing_become_seller: 'Become a Seller',
    landing_browse_category: 'Browse by Category',
    landing_view_all: 'View All →',
    landing_popular_marketplace: 'Popular in Marketplace',
    landing_view_details: 'View Details',
    landing_popular_vehicles: 'Popular Vehicles',
    landing_view_vehicle: 'View Vehicle',
    landing_popular_real_estate: 'Popular Real Estate',
    landing_view_property: 'View Property',
    landing_popular_jobs: 'Popular Job Openings',
    landing_view_job: 'View Job',
    landing_featured_professionals: 'Featured Professionals',
    landing_book_service: 'Book Service',
    open_salary: 'Open Salary',
    employer: 'Employer',
    rating_label: 'Rating',
    hourly_rate: 'Hourly Rate',
    experties: 'Hire Me',
    select_specialty: 'Select Specialty',
    specialty_housekeeper: 'Housekeeper',
    specialty_childcare: 'Childcare Provider',
    specialty_property_manager: 'Property Manager',
    specialty_cook: 'Cook',
    specialty_caretaker: 'Caretaker',
    specialty_electrician: 'Electrician',
    specialty_plumber: 'Plumber',
    specialty_carpenter: 'Carpenter',
    specialty_handyman: 'Handyman',
    specialty_painter: 'Painter',
    specialty_hvac: 'HVAC Specialist',
    specialty_pest_control: 'Pest Control',
    specialty_contractor: 'Contractor',
    specialty_mason: 'Mason',
    specialty_roofer: 'Roofer',
    specialty_drywall: 'Drywall Specialist',
    specialty_flooring: 'Flooring Specialist',
    specialty_landscaper: 'Landscaper',
    specialty_window_installer: 'Window Installer',
    specialty_tiler: 'Tiler',
    specialty_architect: 'Architect',
    specialty_designer: 'Designer',

    // Marketplace Post Forms Labels & Placeholders
    label_service_title: 'Service / Skill Title',
    label_product_name: 'Product Name',
    label_service_name: 'Service Name',
    label_job_title: 'Job Position Title',
    label_property_title: 'Property Title',
    label_vehicle_title: 'Vehicle Title',
    label_listing_title: 'Listing Title',
    placeholder_service_title: 'e.g. Master Pipe Leak Repair',
    placeholder_product_name: 'e.g. Organic Multivitamins Bottle',
    placeholder_service_name: 'e.g. Professional Tax Advisory',
    placeholder_job_title: 'e.g. Senior Software Engineer',
    placeholder_property_title: 'e.g. Cozy 3-Bedroom Suburban House',
    placeholder_vehicle_title: 'e.g. 2022 Honda Civic LX',
    placeholder_listing_title: 'e.g. Vintage Leather Jacket',
    label_price_optional: 'Price / Value Offered ($) (Optional — Leave blank for "Contact for Price")',
    placeholder_price: 'e.g. 50 (Leave blank for call/inquire)',
    label_rate_estimate: 'Service Rate Estimate',
    placeholder_rate_estimate: 'e.g. $45/hour, Flat rate based on scale',
    label_listing_description: 'Listing Description',
    placeholder_description: 'Describe your offer, specifications, terms and options clearly...',
    label_listing_images: 'Listing Images (Max 5)',
    label_select_files_info: 'Select up to 5 images. Stored locally via base64 encoding.',
    btn_publish_listing: '+ Post to EthiZone',
    btn_cancel: 'Cancel',
    btn_save_changes: 'Save Changes',

    // Storefront Tabs & Labels
    home_tab: 'Home',
    about_tab: 'About',
    contact_tab: 'Contact',
    contact_send_msg: '✉️ Send Message / Request',
    contact_follow_us: 'Follow Us On Social Media',
    contact_form_title: '📩 Contact & Request Form',
    contact_your_details: 'Your Contact Details',
    contact_full_name: 'Full Name',
    contact_email_address: 'Email Address',
    contact_phone_number: 'Phone Number',
    contact_req_details: 'Request Details',
    contact_upload_prescription: 'Upload Doctor Prescription (Image or PDF)',
    contact_notes_symptoms: 'Additional Notes / Symptoms',
    contact_pref_date: 'Preferred Date',
    contact_pref_time: 'Preferred Time',
    contact_case_topic: 'Case Details / Consultation Topic',
    contact_tax_year: 'Tax Filing Year',
    contact_upload_tax_docs: 'Upload Documents (W2s, Receipts)',
    contact_prod_item_name: 'Product Item Name / Description',
    contact_pref_size: 'Preferred Size',
    contact_pref_color: 'Preferred Color',
    contact_preorder_items: 'Pre-Order Items List (Names and Quantities)',
    contact_est_pickup_time: 'Estimated Pickup Time',
    contact_msg_details: 'Message details',
    contact_submit_btn: '🚀 Submit Request & Alert',
    contact_submitting: 'Submitting request...',
    email_required_otp: 'Please enter your email address first.',
    otp_sent_success: 'Verification code sent to your email! Please check your inbox.',
    otp_verification_required: 'Please request an email verification code first.',
    enter_verification_code: 'Enter Verification Code (OTP)',
    inquiry_submitted_success: 'Your inquiry has been submitted successfully!',
    contact_grocery_items: 'Grocery Items List (Names and Quantities)',
    contact_grocery_service_type: 'Service Option',
    contact_grocery_service_pickup: 'In-Store Pickup',
    contact_grocery_service_delivery: 'Home Delivery',
    contact_grocery_time: 'Preferred Pickup / Delivery Time',
    contact_grocery_notes: 'Special Instructions / Delivery Address',
    featured_products: 'Featured Products & Services',
    about_business: 'About Our Business',
    location: 'Physical Location',
    classification: 'Classification',
    registered_since: 'Registered Since',
    contact_directly: 'Contact Us Directly',
    middleman_free: 'We offer direct, middleman-free connection. Reach out via call or SMS text message to request services or check product availability.',
    phone_line: 'Direct Phone Line',
    call_now: 'Call Now',
    send_sms: 'Send SMS',
    no_posts_featured: 'No products, services, or job openings posted by this business yet.',
    no_match_catalog: 'No listings matched your store search query.',
    search_in_catalog: 'Search items in this store...',
    catalog_active_listings: 'Catalog & Active Listings',
    skills_tab: 'Skills',
    used_items_tab: 'Used Items',
    shop_tab: 'Shop',
    services_tab: 'Services',
    job_openings_tab: 'Job Openings',
    properties_tab: 'Properties',
    showroom_tab: 'Showroom',
    catalog_tab: 'Catalog',

    // ListingCard Detail Modal
    provider: 'Provider',
    seller: 'Seller',
    handyman_provider: 'Handyman Provider',
    individual_seller: 'Individual Seller',
    requirements: 'Requirements',
    property: 'Property',
    bedrooms: 'Bedrooms',
    bathrooms: 'Bathrooms',
    address_label: 'Address',
    model_year: 'Model Year',
    brand: 'Brand',
    mileage: 'Mileage',
    rates: 'Rates',
    verified_reviews_title: 'Verified Client References & Reviews',
    no_reviews_modal: 'No references left for this provider yet. Be the first to verify and comment!',
    unavailable: 'Unavailable',

    // RatingForm verification
    write_reference: 'Write a Verified Reference',
    prevent_fake: 'To prevent fake reviews, you must provide your email and phone to verify authenticity.',
    rate: 'Rate',
    review: 'review',
    reviews: 'reviews',
    new_status: 'New',
    your_name: 'Your Name',
    email_address: 'Email Address',
    phone_number: 'Phone Number',
    select_channel: 'Select Verification Channel',
    sms_ver: 'SMS Verification',
    email_ver: 'Email Verification',
    get_code: 'Get Verification Code',
    enter_code: 'Enter 4-Digit Code',
    verify_btn: 'Verify',
    rating_score_label: 'Rating Score',
    your_reference_details: 'Your Reference Details',
    describe_experience: 'Describe your service experience with this provider...',
    post_reference: 'Post Reference Rating',

    // Footer & Global layout
    footer_brand_desc: "Connecting you with Ethiopia's finest vendors, artisans, and professionals in one secure ecosystem.",
    marketplace: 'Marketplace',
    company: 'Company',
    about_us: 'About Us',
    privacy_policy: 'Privacy Policy',
    terms_of_service: 'Terms of Service',
    newsletter: 'Newsletter',
    newsletter_desc: 'Stay updated on new stores and exclusive offers.',
    your_email_placeholder: 'Your email',
    footer_copy: '© 2026 Ethizone Marketplace. All rights reserved.',

    // How It Works section
    how_it_works_title: 'How Ethizone Works',
    how_it_works_subtitle: 'Connecting global buyers with verified sellers in a seamless, secure ecosystem.',
    step1_title: '1. Discover',
    step1_desc: 'Browse through thousands of products, services, vehicles, and jobs across 50+ categories and international borders.',
    step2_title: '2. Secure Transaction',
    step2_desc: 'Buy with confidence using our multi-currency secure payment gateway. All transactions are protected and verified.',
    step3_title: '3. Global Delivery',
    step3_desc: 'Track your order from the seller\'s doorstep to yours with our integrated international logistics network.',
    verified_sellers_title: 'Verified Sellers',
    verified_sellers_desc: 'Rigorous identity checks for every vendor.',
    secure_payments_title: 'Secure Payments',
    secure_payments_desc: '256-bit SSL encrypted transactions.',
    support_title: '24/7 Support',
    top_banner_slogan: 'Ethizone: ALL Buyers are Sellers ⇄ All Sellers are Buyers',
    logo_motto_slogan: '"All Buyers Are Sellers"',
    
    // Platform Explanation keys
    about_app_title: 'About EthiZone Marketplace',
    about_app_subtitle: 'The All-in-One Multi-Category Directory & Direct Marketplace',
    about_app_desc: 'EthiZone is designed to empower individuals, store owners, service providers, real estate agencies, automotive dealers, and skilled handymen. Whether you are buying products, hiring professionals, renting property, or selling items, EthiZone provides direct communication with zero middle-man fees.',
    expl_feature1_title: 'Boutiques & Stores',
    expl_feature1_desc: 'Explore retail boutiques, pharmacies, liquor stores, electronics shops, grocery stores, and spare parts dealers with verified seller profiles.',
    expl_feature2_title: 'Vehicles & Properties',
    expl_feature2_desc: 'Browse cars for sale & rent, auto repair workshops, spare parts dealers, residential homes, apartments, and land plots.',
    expl_feature3_title: 'Services & Handymen',
    expl_feature3_desc: 'Connect with certified electricians, plumbers, legal consultants, tax filing offices, beauty salons, and cleaning agencies.',
    expl_feature4_title: 'Direct Call & SMS',
    expl_feature4_desc: 'Contact vendors and service providers directly via phone call or SMS text message with no commission markup.',
    
    // Static Page Titles & Headers
    our_mission: 'Our Mission',
    our_vision: 'Our Vision',
    what_ethizone_provides: 'What EthiZone Provides',
    what_ethizone_does_not: 'What EthiZone Does Not Do',
    send_support_ticket: 'Send Support Ticket',
    support_ticket_submitted: 'Your support ticket has been submitted successfully!',
    platform_disclaimer: 'Platform Disclaimer',
    user_resp_transactions: '1. User Responsibility for Transactions',
    no_auto_guarantee: '2. No Automatic Guarantee',
    age_and_capacity: '3. Age and Capacity',
    limitation_liability: '4. Limitation of Liability',
    
    // Dynamic Filter Keys
    filters: 'Filters',
    reset_filters: 'Reset Filters',
    status: 'Status',
    property_type: 'Property Type',
    bedrooms_filter: 'Bedrooms',
    bathrooms_filter: 'Bathrooms',
    max_price: 'Max Price',
    location_placeholder: 'City, Town, State or Zip...',
    min_year_filter: 'Min Year',
    transmission: 'Transmission',
    fuel_type: 'Fuel Type',
    job_type: 'Job Type',
    work_model: 'Work Model',
    min_salary: 'Min Salary',
    pricing_model: 'Pricing Model',
    min_experience: 'Min Experience',
    availability: 'Availability',
    min_rating: 'Minimum Rating',
    category: 'Category',
    store_type_filter: 'Store Type',
    price_range: 'Price Range',
    
    // Filter Options
    all: 'All',
    res_homes: 'Residential Homes',
    rent_apts: 'Rental Apartments',
    comm_re: 'Commercial Real Estate',
    land_lots: 'Land & Lots',
    house: 'House',
    apartment: 'Apartment',
    penthouse: 'Penthouse',
    office: 'Office',
    land: 'Land',
    automatic: 'Automatic',
    manual: 'Manual',
    gasoline: 'Gasoline',
    diesel: 'Diesel',
    electric: 'Electric',
    hybrid: 'Hybrid',
    full_time: 'Full-Time',
    part_time: 'Part-Time',
    contract: 'Contract',
    freelance: 'Freelance',
    internship: 'Internship',
    onsite: 'On-site',
    remote: 'Remote',
    hourly_rate: 'Hourly Rate',
    fixed_price: 'Fixed Price per Project',
    contact_quote: 'Contact for Quote',
    avail_today: 'Available Today',
    avail_tomorrow: 'Available Tomorrow',
    avail_this_week: 'Available This Week',
    any_rating: 'Any Rating',
    all_stores: 'All Stores',
    verified_partner: 'Verified Partner',
    independent_maker: 'Independent Maker',

    // Events Page Translations
    events: 'Events',
    events_explore_title: 'Explore Events Nearby',
    events_explore_subtitle: 'Find concerts, workshops, community gatherings, sports tournaments, and other events hosted near you.',
    events_search_placeholder: 'Search event title, description or keywords...',
    events_location_placeholder: 'Search City or Location (e.g. Bole)...',
    events_search_btn: 'Search Events',
    events_cat_all: 'All',
    events_entertainment: 'Entertainment',
    events_arts_culture: 'Arts & Culture',
    events_religious: 'Religious',
    events_social: 'Social',
    events_educational: 'Educational',
    events_sports: 'Sports',
    events_charity: 'Charity',
    events_free: 'FREE',
    events_no_found: 'No Events Found',
    events_no_found_desc: "We couldn't find any events matching your selected category or location. Try clearing filters.",
    events_clear_filters: 'Clear Filters',
    events_searching: 'Searching events nearby...',
    events_date: 'Date & Time',
    events_venue: 'Venue Location',
    events_hosted_by: 'Hosted By',
    events_call: 'Call Host',
    events_sms: 'SMS Host',
    events_details_title: 'Event Details',
  },
  am: {
    // Navbar
    stores: 'ሱቆች',
    handymen: 'ቅጠሩኝ',
    services: 'አገልግሎቶች',
    organizations: 'ስራዎች',
    real_estate: 'ቤቶች',
    automotive: 'መኪናዎች',
    used_items: 'ያገለገሉ እቃዎች',
    events: 'ዝግጅቶች',
    events_explore_title: 'አቅራቢያዎ ያሉ ዝግጅቶችን ይመርምሩ',
    events_explore_subtitle: 'ኮንሰርቶችን፣ ወርክሾፖችን፣ ማህበራዊ ስብሰባዎችን፣ የስፖርት ውድድሮችን እና ሌሎች ዝግጅቶችን ያግኙ።',
    events_search_placeholder: 'የዝግጅት ርዕስ፣ መግለጫ ወይም ቁልፍ ቃላትን ይፈልጉ...',
    events_location_placeholder: 'ከተማ ወይም ቦታ ይፈልጉ (ምሳሌ፡ ቦሌ)...',
    events_search_btn: 'ዝግጅቶችን ፈልግ',
    events_cat_all: 'ሁሉም',
    events_entertainment: 'መዝናኛ',
    events_arts_culture: 'ስነ-ጥበብ እና ባህል',
    events_religious: 'ሃይማኖታዊ',
    events_social: 'ማህበራዊ',
    events_educational: 'ትምህርታዊ',
    events_sports: 'ስፖርት',
    events_charity: 'በጎ አድራጎት',
    events_free: 'ነፃ',
    events_no_found: 'ምንም ዝግጅት አልተገኘም',
    events_no_found_desc: 'ከተመረጠው ምድብ ወይም ቦታ ጋር የሚስማማ ምንም ዝግጅት አላገኘንም። ማጣሪያዎቹን አጽድተው እንደገና ይሞክሩ።',
    events_clear_filters: 'ማጣሪያዎችን አጽዳ',
    events_searching: 'አቅራቢያ ያሉ ዝግጅቶችን በመፈለግ ላይ...',
    events_date: 'ቀን እና ሰዓት',
    events_venue: 'የቦታው አድራሻ',
    events_hosted_by: 'አዘጋጅ',
    events_call: 'አዘጋጁን ደውል',
    events_sms: 'መልዕክት (SMS)',
    events_details_title: 'የዝግጅቱ ዝርዝር',
    be_partner: 'አጋር ይሁኑ',
    login: 'ግባ',
    logout: 'ውጣ',
    dashboard: 'ዳሽቦርድ',
    admin_panel: 'አስተዳዳሪ ፔጅ',
    qr_code: '🖨️ QR ኮድ',
    my_web: '🔗 የእኔ ዌብሳይት',
    
    // Home Page
    hero_title: 'ቀጥታ መገናኛ አካባቢያዊ ማውጫ',
    hero_desc: 'የአከባቢ ሱቆችን፣ የመድኃኒት መሸጫዎችን፣ የሙያ አገልግሎቶችን፣ የግብር አማካሪዎችን እና የቤት ጥገና ባለሙያዎችን በአቅራቢያዎ ያግኙ። ያለምንም አማላጅ ክፍያ በስልክ ወይም በፅሁፍ መልእክት ያግኙዋቸው።',
    search_placeholder: 'ምን መፈለግ ይፈልጋሉ? (ምሳሌ፡ መድኃኒት፣ አናፂ፣ ጃኬት...)',
    search_btn: 'ፈልግ',
    all_categories: 'ሁሉም ዘርፎች',
    all_listings: 'ሁሉም የሽያጭ ማስታወቂያዎች',
    active_postings_for: 'ለ "{type}" የተለጠፉ ንቁ ማስታወቂያዎች',
    contact_for_price: 'ለዋጋው ያነጋግሩ',
    salary_offered: 'የሚከፈል ደመወዝ',
    price: 'ዋጋ',
    visit_store: 'ሱቁን ይጎብኙ',
    call: 'ደውል',
    sms: 'መልዕክት (SMS)',
    verified: '✓ የተረጋገጠ',
    no_listings_found: 'ምንም ማስታወቂያዎች አልተገኙም።',
    
    // Dashboard page
    post_marketplace: 'EthiZone ላይ ይለጥፉ',
    listing_title: 'የማስታወቂያው ርዕስ',
    listing_desc: 'የማስታወቂያው ዝርዝር መግለጫ',
    price_val: 'ዋጋ ($)',
    publish_btn: '+ EthiZone ላይ ይለጥፉ',
    visit_storefront_btn: '🔗 ሱቄን ጎብኝ',
    online_status: 'የመስመር ላይ የደንበኛ ስልክ ሁኔታ',
    status_online: '🟢 ክፍት ነው',
    status_offline: '🔴 ዝግ ነው (እረፍት ላይ)',
    manage_nav: 'የሱቅዎን ምናሌ ያብጁ',
    nav_help: 'እስከ 5 የሚደርሱ የውጭ ዌብሳይት ሊንኮችን (ምሳሌ፡ Yelp፣ ስለ እኛ፣ የቀጠሮ ማስያዣ) በሱቅዎ ምናሌ ላይ በቀጥታ ያክሉ።',
    link_label: 'የሊንኩ ስም',
    dest_url: 'የዌብሳይት አድራሻ (URL)',
    add_link: '+ ሊንኩን ጨምር',
    my_listings: 'የእኔ የሽያጭ ማስታወቂያዎች',
    references_feedback: 'የደንበኛ ምስክርነቶች እና አስተያየቶች',
    no_listings_yet: 'እስካሁን ምንም ማስታወቂያ አልለጠፉም።',
    no_refs_yet: 'እስካሁን ምንም የደንበኛ አስተያየት የለም።',
    verified_reviews_desc: 'ከድርጅትዎ ጋር የአገልግሎት ግንኙነት ያደረጉ ደንበኞች የሰጡት አስተያየት እና ባለኮከብ ደረጃዎች ከዚህ በታች ቀርበዋል።',
    
    // Static Pages (User can update these later)
    about_us_content: 'EthiZone ግለሰቦች፣ ሱቆች፣ አገልግሎት ሰጪዎች፣ ባለሙያዎች እና የንግድ ድርጅቶች በአንድ የታመነ ዲጂታል የገበያ ቦታ ላይ እንዲገናኙ የሚረዳ መድረክ ነው። ሁሉም ገዢዎች ሻጮች ናቸው በሚለው ሃሳብ ዙሪያ የተገነባ ነው።',
    contact_tab_content: 'እገዛ ይፈልጋሉ፣ ጥያቄ አለዎት ወይስ ችግርን ሪፖርት ማድረግ ይፈልጋሉ? የEthiZone የድጋፍ ቡድንን ያነጋግሩ። የድጋፍ ኢሜይል፡ support@ethizone.com | የንግድ እና አጋርነት ኢሜይል፡ partners@ethizone.com | ደህንነት እና ሪፖርት፡ safety@ethizone.com። የድጋፍ ምላሽ ጊዜ፡ በተቻለ ፍጥነት ምላሽ ለመስጠት እንጥራለን።',
    privacy_policy_content: 'EthiZone የተጠቃሚዎችን ግላዊነት ያከብራል እንዲሁም የግል መረጃዎችን ለመጠበቅ ቁርጠኛ ነው። ይህ የግላዊነት ፖሊሲ ተጠቃሚዎች EthiZoneን ሲጎበኙ መረጃ እንዴት እንደሚሰበሰብ እና ጥቅም ላይ እንደሚውል ያብራራል።',
    terms_of_service_content: 'EthiZoneን በመጠቀም በአገልግሎት ውሎቻችን ለመገዛት ተስማምተዋል። EthiZone ዲጂታል የገበያ ቦታ እና የቴክኖሎጂ መድረክ ነው። ሌላ የጽሁፍ ዋስትና በግልጽ ካልተሰጠ በስተቀር EthiZone በተጠቃሚዎች ግብይት ውስጥ ሻጭ፣ ገዢ፣ አከራይ፣ ተከራይ፣ ቀጣሪ ወይም ወኪል አይደለም።',
    legal_disclaimer_title: '⚠️ ጠቃሚ ህጋዊ ማሳሰቢያ',
    legal_disclaimer: 'ለማንኛውም የገንዘብ ዝውውር EthiZone ሀላፊነት አይወስድም። ሻጩ ለምርቶቹ እና ለአገልግሎቶቹ ሙሉ ሀላፊነቱን ይወስዳል።',
    
    // Storefront page
    catalog_listings: 'የማስታወቂያዎች ዝርዝር',
    no_posts_yet: 'ይህ ድርጅት እስካሁን ምንም ምርት፣ አገልግሎት ወይም የስራ ቅጥር ማስታወቂያ አልለጠፈም።',
    no_match_search: 'ከፍለጋዎ ጋር የሚዛመድ ማስታወቂያ አልተገኘም።',
    search_in_store: '🔍 በዚህ ሱቅ ውስጥ ምርቶችን ፈልግ...',
    cust_references: 'የደንበኞች ምስክርነት',
    no_reviews_yet: 'ምንም ምስክርነት እስካሁን አልተለጠፈም። የመጀመሪያውን አስተያየት ይስጡ!',
    direct_line: 'ቀጥታ ስልክ መስመር',
    submit_ref: 'ምስክርነት አስገባ',
    rating_score: 'ደረጃ (ከ1-5 ኮከብ)',
    comment: 'የአስተያየት መልእክት',
    submit_btn: 'አስተያየት አስገባ',
    
    // Login / Register page
    create_account: 'የባለሙያ አካውንት ይፍጠሩ',
    register_desc: 'ምርቶችዎን ለመዘርዘር፣ ክህሎትዎን ለማስተዋወቅ እና ከአከባቢዎ ደንበኞች ጋር በቀጥታ ለመገናኘት ይመዝገቡ።',
    username: 'የተጠቃሚ ስም',
    email: 'ኢሜይል አድራሻ',
    phone: 'የስልክ ቁጥር (ደንበኞች የሚደውሉበት)',
    password: 'የይለፍ ቃል',
    account_role: 'የአካውንት አይነት / ዓላማ',
    role_individual: 'ግለሰብ ሻጭ (ያገለገሉ እቃዎች፣ የቤት እቃዎች)',
    role_handyman: 'ባለሙያ / የቤት አገልግሎት ሰጪ (አናፂ፣ ቧንቧ ሠራተኛ)',
    role_business: 'አካባቢያዊ ንግድ / የሱቅ ባለቤት',
    business_type: 'የንግድ ዘርፍ አይነት',
    select_type: 'የንግድ አይነት ይምረጡ',
    store: 'ሱቅ / የችርቻሮ መሸጫ',
    service: 'አገልግሎት / አማካሪ',
    organization: 'ድርጅት / ኩባንያ',
    real_estate_type: 'ቤትና መሬት አከራይ / ሻጭ',
    automotive_type: 'መኪና አከራይ / ሻጭ',
    register_btn: 'ይመዝገቡ',
    already_account: 'አካውንት አለዎት? እዚህ ይግቡ',
    login_account: 'ወደ አካውንትዎ ይግቡ',
    no_account: 'አዲስ ተጠቃሚ ነዎት? እዚህ ይመዝገቡ',

    // Registration page specific (4-color unified theme)
    reg_title: 'አካውንት ይፍጠሩ',
    reg_subtitle: 'ቀጥተኛ ግንኙነት ያለው የአካባቢ መገበያያ ይቀላቀሉ',
    reg_fullname: 'ሙሉ ስም',
    reg_email: 'ኢሜይል አድራሻ',
    reg_phone: 'ስልክ ቁጥር (ለቀጥታ የደንበኞች ጥሪ)',
    reg_password: 'የይለፍ ቃል',
    reg_account_type: 'የአካውንት አይነት',
    reg_individual: 'ግለሰብ',
    reg_sell_personal: 'ያገለገሉ ዕቃዎችን ለመሸጥ',
    reg_handyman: 'ባለሙያ',
    reg_list_skills: 'የእጅ ስራዎችን ለመዘርዘር',
    reg_business: 'ድርጅት',
    reg_stores_agencies: 'ሱቆች እና ኤጀንሲዎች',
    reg_business_type: 'የንግድ ዘርፍ',
    reg_store: 'ሱቅ',
    reg_sells_products: 'ምርቶችን የሚሸጥ',
    reg_service: 'አገልግሎት',
    reg_offers_services: 'አገልግሎት የሚሰጥ',
    reg_organization: 'ኩባንያ',
    reg_hiring_jobs: 'ቅጥር እና ስራዎች',
    reg_real_estate: 'ቤትና መሬት',
    reg_housing_listings: 'የቤትና ቦታዎች ማስታወቂያ',
    reg_automotive: 'መኪና',
    reg_car_listings: 'የመኪና ሽያጭ/ኪራይ',
    reg_category_type: 'የንግድ ዘርፍ ምድብ',
    reg_btn: 'አሁን ይመዝገቡ',
    reg_creating: 'በመመዝገብ ላይ...',
    reg_login_here: 'እዚህ ይግቡ',
    reg_already_have: 'ቀደም ሲል አካውንት አለዎት?',
    reg_event: 'ዝግጅቶች',
    reg_posts_events: 'ዝግጅት ለማስተናገድ',

    // Category names
    boutique: 'ቡቲክ',
    pharmacy: 'ፋርማሲ',
    liquor_store: 'የመጠጥ ሱቅ',
    grocery_store: 'የግሮሰሪ ሱቅ',
    entertainment: 'መዝናኛ',
    arts_slots_culture: 'ስነ-ጥበብ እና ባህል',
    religious: 'ሃይማኖታዊ',
    social: 'ማህበራዊ',
    educational: 'ትምህርታዊ',
    sports: 'ስፖርት',
    charity: 'በጎ አድራጎት',
    electronics_shop: 'የኤሌክትሮኒክስ ሱቅ',
    bookstore: 'የመጽሐፍ ሱቅ',
    furniture: 'የቤት ዕቃዎች',
    hardware_store: 'የሃርድዌር ሱቅ',
    cafe_slots_restaurant: 'ካፌ እና ሬስቶራንት',
    cafe_restaurant: 'ካፌ እና ሬስቶራንት',
    jewelry_slots_accessories: 'ጌጣጌጥ እና አልባሳት',
    jewelry_accessories: 'ጌጣጌጥ እና አልባሳት',
    gift_slots_toy_shop: 'የስጦታ እና መጫወቻ ሱቅ',
    gift_toy: 'የስጦታ እና መጫወቻ ሱቅ',
    other_store: 'ሌላ ሱቅ',
    law_office: 'የህግ ቢሮ',
    tax_office: 'የታክስ አገልግሎት',
    dental_clinic: 'የጥርስ ህክምና ክሊኒክ',
    clinic: 'ክሊኒክ',
    consulting_firm: 'የማማከር አገልግሎት',
    cleaning_agency: 'የፅዳት አገልግሎት',
    beauty_salon: 'የውበት ሳሎን',
    tech_corporation: 'የቴክኖሎጂ ኩባንያ',
    construction_company: 'የኮንስትራክሽን ኩባንያ',
    healthcare_group: 'የጤና ጥበቃ ድርጅት',
    educational_institution: 'የትምህርት ተቋም',
    non_profit_org: 'ሰብአዊ ድርጅት',
    other: 'ሌላ',
    residential_homes: 'የመኖሪያ ቤቶች',
    rental_apartments: 'የኪራይ አፓርታማዎች',
    commercial_real_estate: 'የንግድ ህንፃዎች',
    land_slots: 'ባዶ ቦታዎች',
    used_car_dealership: 'ያገለገሉ መኪናዎች መሸጫ',
    car_rental_service: 'የመኪና ኪራይ አገልግሎት',
    auto_repair_workshop: 'የመኪና ጥገና ጋራዥ',
    spare_parts_dealer: 'የመኪና መለዋወጫ መሸጫ',
    
    // Grocery categories
    grocery_home: 'መነሻ',
    grocery_all_products: 'ሁሉም ምርቶች',
    grocery_fruits_vegetables: 'አትክልትና ፍራፍሬ',
    grocery_meat_fish: 'ስጋና ዓሳ',
    grocery_dairy_eggs: 'የወተት ተዋጽኦና ዕንቁላል',
    grocery_bakery: 'ዳቦና ኬክ',
    grocery_pantry: 'ደረቅ ምግቦች',
    grocery_drinks: 'መጠጦች',
    grocery_household: 'የቤት ውስጥ ዕቃዎች',
    grocery_baby_personal_care: 'የህጻናትና የግል ንጽህና',
    grocery_new_arrivals: 'አዳዲስ ምርቶች',
    grocery_on_sale: 'በቅናሽ የቀረቡ',
    grocery_about_us: 'ስለ እኛ',
    
    // Liquor categories
    liquor_home: 'መነሻ',
    liquor_all_drinks: 'ሁሉም መጠጦች',
    liquor_wine: 'ወይን',
    liquor_beer: 'ቢራ',
    liquor_whiskey: 'ዊስኪ',
    liquor_vodka: 'ቮድካ',
    liquor_gin: 'ጂን',
    liquor_rum: 'ረም',
    liquor_tequila: 'ተኪላ',
    liquor_champagne: 'ሻምፓኝ',
    liquor_mixers: 'ማደባለቂያዎች',
    liquor_snacks: 'መክሰስ',
    liquor_gift_sets: 'የስጦታ እቃዎች',
    liquor_new_arrivals: 'አዳዲስ ምርቶች',
    liquor_on_sale: 'በቅናሽ የቀረቡ',
    liquor_about_us: 'ስለ እኛ',

    // Electronics categories
    electronics_home: 'መነሻ',
    electronics_all_electronics: 'ሁሉም ኤሌክትሮኒክስ',
    electronics_phones: 'ስልኮች',
    electronics_laptops: 'ላፕቶፖች',
    electronics_tv_audio: 'ቲቪ እና ኦዲዮ',
    electronics_gaming: 'ጌሚንግ',
    electronics_accessories: 'መለዋወጫዎች',
    electronics_smart_devices: 'ስማርት መሳሪያዎች',
    electronics_appliances: 'የቤት ውስጥ መገልገያዎች',
    electronics_new_arrivals: 'አዳዲስ ምርቶች',
    electronics_on_sale: 'በቅናሽ የቀረቡ',
    electronics_refurbished: 'የታደሱ ምርቶች',
    electronics_warranty: 'ዋስትና ያላቸው',
    electronics_about_us: 'ስለ እኛ',

    // Law Office categories
    law_home: 'መነሻ',
    law_about_us: 'ስለ እኛ',
    law_all_services: 'የህግ አገልግሎቶች',
    law_family_law: 'የቤተሰብ ህግ',
    law_business_law: 'የንግድ ህግ',
    law_immigration: 'ኢሚግሬሽን',
    law_real_estate_law: 'የሪል እስቴት ህግ',
    law_contract_review: 'የውል ስምምነት ግምገማ',
    law_consultation: 'ማማከር',
    law_attorneys: 'ጠበቆች',
    law_book_appointment: 'ቀጠሮ መያዝ',
    law_contact: 'እኛን ለማግኘት',
    schedule_appointment: 'ቀጠሮ ያስይዙ',

    // Tax Office categories
    tax_home: 'መነሻ',
    tax_about_us: 'ስለ እኛ',
    tax_all_services: 'የታክስ አገልግሎቶች',
    tax_personal_tax: 'የግል ታክስ',
    tax_business_tax: 'የንግድ ድርጅት ታክስ',
    tax_vat: 'ተጨማሪ እሴት ታክስ (VAT)',
    tax_payroll_tax: 'የደመወዝ ታክስ',
    tax_accounting: 'ሂሳብ አያያዝ',
    tax_bookkeeping: 'የሂሳብ መዛግብት አያያዝ',
    tax_consultation: 'የታክስ ማማከር',
    tax_book_appointment: 'ቀጠሮ መያዝ',
    tax_contact: 'እኛን ለማግኘት',

    // Clinic categories
    clinic_home: 'መነሻ',
    clinic_about_us: 'ስለ እኛ',
    clinic_all_services: 'አገልግሎቶች',
    clinic_doctors: 'ሐኪሞች',
    clinic_general_checkup: 'አጠቃላይ ምርመራ',
    clinic_laboratory: 'ላቦራቶሪ',
    clinic_pharmacy: 'ፋርማሲ',
    clinic_dental: 'የጥርስ ሕክምና',
    clinic_eye_care: 'የዓይን ሕክምና',
    clinic_womens_health: 'የሴቶች ጤና',
    clinic_book_appointment: 'ቀጠሮ መያዝ',
    clinic_contact: 'እኛን ለማግኘት',

    // Cleaning categories
    cleaning_home: 'መነሻ',
    cleaning_about_us: 'ስለ እኛ',
    cleaning_all_services: 'አገልግሎቶች',
    cleaning_house_cleaning: 'የቤት ማጽዳት',
    cleaning_office_cleaning: 'የቢሮ ማጽዳት',
    cleaning_deep_cleaning: 'ጥልቅ ማጽዳት',
    cleaning_move_in_out: 'ሲገቡ/ሲወጡ ማጽዳት',
    cleaning_carpet_cleaning: 'ምንጣፍ ማጽዳት',
    cleaning_disinfection: 'ፀረ-ተህዋስያን መርጨት',
    cleaning_book_now: 'አሁኑኑ ይያዙ',
    cleaning_contact: 'እኛን ለማግኘት',

    // Beauty Salon categories
    beauty_home: 'መነሻ',
    beauty_about_us: 'ስለ እኛ',
    beauty_all_services: 'አገልግሎቶች',
    beauty_hair: 'የፀጉር ሥራ',
    beauty_makeup: 'ሜካፕ',
    beauty_nails: 'የጥፍር ሥራ',
    beauty_facial: 'የፊት ህክምና',
    beauty_barber: 'ፀጉር አስተካካይ',
    beauty_spa: 'ስፓ (Spa)',
    beauty_packages: 'ጥቅል አገልግሎቶች',
    beauty_gallery: 'ማሳያ (Gallery)',
    beauty_book_appointment: 'ቀጠሮ መያዝ',
    beauty_contact: 'እኛን ለማግኘት',

    // Login page specific
    login_title_pw: 'እንኳን ደህና መጡ',
    login_title_otp: 'በማረጋገጫ ኮድ መግቢያ',
    login_subtitle_pw: 'ዳሽቦርድዎን እና ማስታወቂያዎችዎን ለማየት ይግቡ',
    login_subtitle_otp: 'ባለ 6-አሃዝ መግቢያ ኮድ ለመቀበል ኢሜይልዎን ያስገቡ',
    login_email: 'ኢሜይል አድራሻ',
    login_password: 'የይለፍ ቃል',
    login_forgot: 'የይለፍ ቃል ረሱ? በአንድ ጊዜ ኮድ ይግቡ',
    login_btn_loading: 'በመግባት ላይ...',
    login_btn_pw: 'ይግቡ',
    login_otp_label: 'ባለ 6-አሃዝ ኮድ',
    login_expires_in: 'ኮዱ የሚያበቃው ከ',
    login_seconds: 'ሰከንዶች በኋላ ነው',
    login_expired: 'ኮዱ አልፎበታል።',
    login_resend: 'ኮዱን ድጋሚ ላክ',
    login_processing: 'በማቀናበር ላይ...',
    login_verify_signin: 'አረጋግጥና ግባ',
    login_send_otp: 'የአንድ ጊዜ ኮድ ላክ',
    login_back_standard: 'ወደ መደበኛ የይለፍ ቃል መግቢያ ተመለስ',
    login_no_account: 'አካውንት የለዎትም?',
    login_register_here: 'እዚህ ይመዝገቡ',

    // Homepage Landing specific
    landing_hero_title: 'የሚፈልጉትን ሁሉ በአንድ መገበያያ',
    landing_hero_desc: 'ምርቶችን ይግዙ፣ ሱቆችን ያግኙ፣ አገልግሎቶችን ያግኙ፣ ቤቶችን፣ መኪናዎችን፣ ስራዎችን እና ሌሎችንም በአለማችን ሁለገብ እና ደህንነቱ የተጠበቀ መገበያያ ላይ ያስሱ።',
    landing_start_shopping: 'መገበያየት ጀምር',
    landing_become_seller: 'ሻጭ ሁን',
    landing_browse_category: 'በምድብ ፈልግ',
    landing_view_all: 'ሁሉንም አሳይ →',
    landing_popular_marketplace: 'በመገበያያው ተወዳጅ የሆኑ',
    landing_view_details: 'ዝርዝር መረጃ',
    landing_popular_vehicles: 'ተወዳጅ መኪናዎች',
    landing_view_vehicle: 'መኪናውን አሳይ',
    landing_popular_real_estate: 'ተወዳጅ ቤትና ቦታዎች',
    landing_view_property: 'ንብረቱን አሳይ',
    landing_popular_jobs: 'ተወዳጅ የስራ ማስታወቂያዎች',
    landing_view_job: 'ስራውን አሳይ',
    landing_featured_professionals: 'የተመረጡ ባለሙያዎች',
    landing_book_service: 'አገልግሎት አስይዝ',
    open_salary: 'ክፍት ደሞዝ',
    employer: 'ቀጣሪ',
    rating_label: 'ደረጃ',
    hourly_rate: 'የሰዓት ዋጋ',
    experties: 'ቅጠረኝ (Hire Me)',
    select_specialty: 'ሙያ ይምረጡ',
    specialty_housekeeper: 'የቤት ሰራተኛ',
    specialty_childcare: 'የህጻናት ጠባቂ',
    specialty_property_manager: 'የንብረት አስተዳዳሪ',
    specialty_cook: 'ወጥ ቤት (አንጋሽ)',
    specialty_caretaker: 'ጠባቂ (ተንከባካቢ)',
    specialty_electrician: 'ኤሌክትሪሻን',
    specialty_plumber: 'ቧንቧ ሰራተኛ',
    specialty_carpenter: 'አናፂ',
    specialty_handyman: 'ረዳት ሰራተኛ',
    specialty_painter: 'ቀለም ቀቢ',
    specialty_hvac: 'የአየር ማቀዝቀዣ ባለሙያ',
    specialty_pest_control: 'የተባይ ማጥፊያ',
    specialty_contractor: 'ተቋራጭ',
    specialty_mason: 'ድንጋይ ጠራቢ (ግንበኛ)',
    specialty_roofer: 'የጣሪያ ሰራተኛ',
    specialty_drywall: 'የጂፕሰም ባለሙያ',
    specialty_flooring: 'የወለል ንጣፍ ባለሙያ',
    specialty_landscaper: 'የአትክልት ባለሙያ',
    specialty_window_installer: 'የመስኮት ገጣሚ',
    specialty_tiler: 'ሴራሚክ ገጣሚ',
    specialty_architect: 'አርክቴክት',
    specialty_designer: 'ዲዛይነር',

    // Marketplace Post Forms Labels & Placeholders
    label_service_title: 'የአገልግሎት / የሙያ ርዕስ',
    label_product_name: 'የምርት ስም',
    label_service_name: 'የአገልግሎት ስም',
    label_job_title: 'የስራ መደብ ርዕስ',
    label_property_title: 'የንብረት ርዕስ',
    label_vehicle_title: 'የመኪና ርዕስ',
    label_listing_title: 'የማስታወቂያ ርዕስ',
    placeholder_service_title: 'ምሳሌ፡ የቧንቧ ጥገና',
    placeholder_product_name: 'ምሳሌ፡ ኦርጋኒክ ቫይታሚን',
    placeholder_service_name: 'ምሳሌ፡ የታክስ ማማከር አገልግሎት',
    placeholder_job_title: 'ምሳሌ፡ ሲኒየር ሶፍትዌር መሃንዲስ',
    placeholder_property_title: 'ምሳሌ፡ ባለ 3 መኝታ ቤት',
    placeholder_vehicle_title: 'ምሳሌ፡ 2022 ሆንዳ ሲቪክ',
    placeholder_listing_title: 'ምሳሌ፡ ቪንቴጅ ጃኬት',
    label_price_optional: 'ዋጋ / ክፍያ ($) (አማራጭ — ለ"ዋጋ ለመደራደር" ባዶ ይተውት)',
    placeholder_price: 'ምሳሌ፡ 50 (ለመደወል/ለመጠየቅ ባዶ ይተውት)',
    label_rate_estimate: 'የአገልግሎት ዋጋ ግምት',
    placeholder_rate_estimate: 'ምሳሌ፡ $45 በሰዓት ወይም ቋሚ ዋጋ',
    label_listing_description: 'የማስታወቂያው ዝርዝር መግለጫ',
    placeholder_description: 'የሚያቀርቡትን አገልግሎት፣ ዝርዝር ሁኔታዎች፣ ውሎች እና አማራጮች በግልጽ ያብራሩ...',
    label_listing_images: 'የማስታወቂያ ምስሎች (ከ 5 አይበልጥም)',
    label_select_files_info: 'እስከ 5 ምስሎችን ይምረጡ። በ base64 ኢንኮዲንግ አማካኝነት በሎካል ይቀመጣሉ።',
    btn_publish_listing: '+ EthiZone ላይ ይለጥፉ',
    btn_cancel: 'ሰርዝ',
    btn_save_changes: 'ለውጦችን አስቀምጥ',

    // Storefront Tabs & Labels
    home_tab: 'መነሻ',
    about_tab: 'ስለ እኛ',
    contact_tab: 'ግንኙነት',
    contact_send_msg: '✉️ መልዕክት / ጥያቄ ይላኩ',
    contact_follow_us: 'በማህበራዊ ሚዲያ ይከተሉን',
    contact_form_title: '📩 የጥያቄ እና ግንኙነት ፎርም',
    contact_your_details: 'የእርስዎ የመገናኛ ዝርዝሮች',
    contact_full_name: 'ሙሉ ስም',
    contact_email_address: 'ኢሜል አድራሻ',
    contact_phone_number: 'ስልክ ቁጥር',
    contact_req_details: 'የጥያቄው ዝርዝር',
    contact_upload_prescription: 'የሃኪም ማዘዣ ያስገቡ (ምስል ወይም PDF)',
    contact_notes_symptoms: 'ተጨማሪ ማስታወሻዎች / ምልክቶች',
    contact_pref_date: 'የሚመርጡት ቀን',
    contact_pref_time: 'የሚመርጡት ሰዓት',
    contact_case_topic: 'የጉዳዩ ዝርዝር / የማማከር ርዕስ',
    contact_tax_year: 'የግብር ማስገቢያ ዓመት',
    contact_upload_tax_docs: 'ሰነዶችን ያስገቡ (W2s፣ ደረሰኞች)',
    contact_prod_item_name: 'የምርቱ ስም / መግለጫ',
    contact_pref_size: 'የሚመርጡት መጠን (ሳይዝ)',
    contact_pref_color: 'የሚመርጡት ቀለም',
    contact_preorder_items: 'ቅድመ-ትዕዛዝ እቃዎች ዝርዝር (ስም እና ብዛት)',
    contact_est_pickup_time: 'የመውሰጃ ግምታዊ ሰዓት',
    contact_msg_details: 'የመልዕክቱ ዝርዝር',
    contact_submit_btn: '🚀 ጥያቄውን ያስገቡ',
    contact_submitting: 'ጥያቄው እየገባ ነው...',
    email_required_otp: 'እባክዎ መጀመሪያ የኢሜል አድራሻዎን ያስገቡ።',
    otp_sent_success: 'የማረጋገጫ ኮድ ወደ ኢሜልዎ ተልኳል! እባክዎ የኢሜል ሳጥንዎን ያረጋግጡ።',
    otp_verification_required: 'እባክዎ መጀመሪያ የኢሜል ማረጋገጫ ኮድ ይጠይቁ።',
    enter_verification_code: 'የማረጋገጫ ኮድ ያስገቡ (OTP)',
    inquiry_submitted_success: 'የጥያቄ መልዕክትዎ በተሳካ ሁኔታ ተልኳል!',
    contact_grocery_items: 'የግሮሰሪ እቃዎች ዝርዝር (ስም እና ብዛት)',
    contact_grocery_service_type: 'የአገልግሎት አማራጭ',
    contact_grocery_service_pickup: 'ሱቅ መጥቶ መውሰድ',
    contact_grocery_service_delivery: 'ቤት ማድረስ',
    contact_grocery_time: 'የመውሰጃ / የማድረሻ ሰዓት',
    contact_grocery_notes: 'ልዩ መመሪያዎች / የማድረሻ አድራሻ',
    featured_products: 'የተመረጡ ምርቶች እና አገልግሎቶች',
    about_business: 'ስለ ድርጅታችን',
    location: 'አድራሻ',
    classification: 'ምድብ',
    registered_since: 'ከተመዘገበበት ጊዜ',
    contact_directly: 'በቀጥታ ያግኙን',
    middleman_free: 'ያለ አማላጅ በቀጥታ መገናኘት የሚያስችል ሲስተም አዘጋጅተናል። አገልግሎቶችን ለመጠየቅ ወይም የእቃዎችን መኖር ለማረጋገጥ በስልክ ወይም በፅሁፍ መልእክት ያግኙን።',
    phone_line: 'ቀጥታ ስልክ መስመር',
    call_now: 'አሁን ይደውሉ',
    send_sms: 'ፅሁፍ ላክ',
    no_posts_featured: 'ይህ ድርጅት እስካሁን ምንም ምርት፣ አገልግሎት ወይም የስራ ቅጥር ማስታወቂያ አልለጠፈም።',
    no_match_catalog: 'ከፍለጋዎ ጋር የሚዛመድ ማስታወቂያ አልተገኘም።',
    search_in_catalog: 'በዚህ ሱቅ ውስጥ ፈልግ...',
    catalog_active_listings: 'የማስታወቂያዎች ዝርዝር',
    skills_tab: 'ሙያ',
    used_items_tab: 'ያገለገሉ እቃዎች',
    shop_tab: 'ሱቅ',
    services_tab: 'አገልግሎቶች',
    job_openings_tab: 'የስራ ቅጥሮች',
    properties_tab: 'ቤትና መሬት',
    showroom_tab: 'መኪናዎች',
    catalog_tab: 'ዝርዝር',

    // ListingCard Detail Modal
    provider: 'አቅራቢ',
    seller: 'ሻጭ',
    handyman_provider: 'የቤት አገልግሎት ሰጪ',
    individual_seller: 'ግለሰብ ሻጭ',
    requirements: 'መመዘኛዎች',
    property: 'ንብረት',
    bedrooms: 'መኝታ ቤቶች',
    bathrooms: 'መታጠቢያ ቤቶች',
    address_label: 'አድራሻ',
    model_year: 'የተመረተበት ዓመት',
    brand: 'ብራንድ',
    mileage: 'ኪሎሜትር',
    rates: 'ዋጋ',
    verified_reviews_title: 'የተረጋገጡ የደንበኞች ምስክርነቶች እና ደረጃዎች',
    no_reviews_modal: 'እስካሁን ምንም አስተያየት የለም። የመጀመሪያውን አስተያየት በመስጠት ይረዱ!',
    unavailable: 'አይገኝም',

    // RatingForm verification
    write_reference: 'የተረጋገጠ ምስክርነት ይጻፉ',
    prevent_fake: 'ሐሰተኛ አስተያየቶችን ለመከላከል ኢሜይልዎን እና ስልክ ቁጥርዎን ማስገባት አለብዎት።',
    rate: 'ደረጃ',
    review: 'አስተያየት',
    reviews: 'አስተያየቶች',
    new_status: 'አዲስ',
    your_name: 'ስምዎ',
    email_address: 'ኢሜይል አድራሻ',
    phone_number: 'ስልክ ቁጥር',
    select_channel: 'የማረጋገጫ መስመር ይምረጡ',
    sms_ver: 'በፅሁፍ መልእክት (SMS)',
    email_ver: 'በኢሜይል',
    get_code: 'የማረጋገጫ ኮድ አግኝ',
    enter_code: 'ባለ 4 ዲጂት ኮድ ያስገቡ',
    verify_btn: 'አረጋግጥ',
    rating_score_label: 'ደረጃ',
    your_reference_details: 'የምስክርነትዎ ዝርዝር አስተያየት',
    describe_experience: 'ከዚህ አገልግሎት ሰጪ ጋር የነበረዎትን ልምድ ያብራሩ...',
    post_reference: 'ምስክርነቱን ይለጥፉ',

    // Footer & Global layout
    footer_brand_desc: 'ከኢትዮጵያ ምርጥ ነጋዴዎች፣ ባለሙያዎች እና የእጅ ጥበብ ባለሙያዎች ጋር በአንድ አስተማማኝ ስርዓት ውስጥ እናገናኝዎታለን።',
    marketplace: 'የሽያጭ ቦታ',
    company: 'ኩባንያ',
    about_us: 'ስለ እኛ',
    privacy_policy: 'የግላዊነት ፖሊሲ',
    terms_of_service: 'የአገልግሎት ውሎች',
    newsletter: 'ጋዜጣ',
    newsletter_desc: 'ስለ አዳዲስ መደብሮች እና ልዩ ቅናሾች መረጃ ያግኙ።',
    your_email_placeholder: 'የእርስዎ ኢሜይል',
    footer_copy: '© 2026 ኢትዮዞን ማርኬትፕሌስ። መብቱ በህግ የተጠበቀ ነው።',

    // How It Works section
    how_it_works_title: 'ኢትዮዞን እንዴት ይሰራል?',
    how_it_works_subtitle: 'ዓለም አቀፍ ገዢዎችን ከተረጋገጡ ሻጮች ጋር ያለምንም እንከን በደህንነት ያገናኛል።',
    step1_title: '1. መፈለግ',
    step1_desc: 'በሺዎች የሚቆጠሩ ምርቶችን፣ አገልግሎቶችን፣ መኪናዎችን እና ስራዎችን ከ50 በላይ በሆኑ ምድቦች እና በሀገር አቀፍ ደረጃ ያስሱ።',
    step2_title: '2. አስተማማኝ ክፍያ',
    step2_desc: 'በእኛ አስተማማኝ የክፍያ አማራጭ በመጠቀም በልበ ሙሉነት ይግዙ። ሁሉም የክፍያ ስራዎች የተጠበቁ እና የተረጋገጡ ናቸው።',
    step3_title: '3. ቀጥተኛ ግንኙነት',
    step3_desc: 'ያለምንም አማላጅ ምርቶችን ከሻጩ በቀጥታ በስልክ ወይም በፅሁፍ መልእክት ያግኙ።',
    verified_sellers_title: 'የተረጋገጡ ሻጮች',
    verified_sellers_desc: 'ለእያንዳንዱ ሻጭ ጥብቅ የማንነት ማረጋገጫ ይደረጋል።',
    secure_payments_title: 'አስተማማኝ ክፍያዎች',
    secure_payments_desc: 'በከፍተኛ የደህንነት ስርዓት የተጠበቁ የክፍያ ግብይቶች።',
    support_title: '24/7 ድጋፍ',
    support_desc: 'በተለያዩ ቋንቋዎች የሚሰጥ ዓለም አቀፍ የደንበኛ እገዛ።',
    top_banner_slogan: 'ኢትዮዞን፡ ሁሉም ገዢዎች ሻጮች ናቸው ⇄ ሁሉም ሻጮች ገዢዎች ናቸው',
    logo_motto_slogan: '"ሁሉም ገዢዎች ሻጮች ናቸው"',
    
    // Platform Explanation keys
    about_app_title: 'ስለ EthiZone መገበያያ ገጽ',
    about_app_subtitle: 'ሁሉንም በአንድ ያካተተ አጠቃላይ የንግድና አገልግሎት ማውጫ',
    about_app_desc: 'EthiZone ግለሰቦችን፣ የሱቅ ባለቤቶችን፣ የአገልግሎት ሰጪዎችን፣ የሪል እስቴት እና የመኪና አከፋፋዮችን እንዲሁም ባለሙያዎችን በቀጥታ የሚያገናኝ ዘመናዊ መድረክ ነው። ያለምንም አማላጅ ክፍያ በስልክ ወይም በፅሁፍ መልእክት በመገናኘት ይገበያዩ፣ አገልግሎት ያግኙ ወይም ስራ ይቅጠሩ።',
    expl_feature1_title: 'ሱቆች እና የችርቻሮ ምርቶች',
    expl_feature1_desc: 'ቡቲኮችን፣ መድኃኒት ቤቶችን፣ ኤሌክትሮኒክስ ሱቆችን፣ የምግብ ሸቀጣ ሸቀጥና የመኪና መለዋወጫ መሸጫዎችን ከተረጋገጡ መገለጫዎች ጋር ያግኙ።',
    expl_feature2_title: 'መኪናዎች እና ቤቶች',
    expl_feature2_desc: 'የሚሸጡና የሚከራዩ መኪናዎችን፣ ጋራዦችን፣ የመኪና መለዋወጫዎችን፣ የመኖሪያ ቤቶችን፣ አፓርታማዎችንና ቦታዎችን ይመልከቱ።',
    expl_feature3_title: 'አገልግሎቶች እና የቤት ጥገና',
    expl_feature3_desc: 'ከተረጋገጡ የኤሌክትሪክ፣ የቧንቧ፣ የህግ፣ የታክስ፣ የውበት እና የፅዳት ባለሙያዎች ጋር በቀጥታ ይገናኙ።',
    expl_feature4_title: 'ቀጥታ ስልክ እና መልዕክት (SMS)',
    expl_feature4_desc: 'ያለምንም አላስፈላጊ ደላላ ወይም ተጨማሪ ክፍያ ከሻጮችና አገልግሎት ሰጪዎች ጋር በቀጥታ በስልክ ደውለው ወይም መልዕክት ልከው ይነጋገሩ።',
    
    // Static Page Titles & Headers
    our_mission: 'ተልዕኳችን',
    our_vision: 'ራዕያችን',
    what_ethizone_provides: 'EthiZone የሚያቀርባቸው አገልግሎቶች',
    what_ethizone_does_not: 'EthiZone የማያደርጋቸው ነገሮች',
    send_support_ticket: 'የድጋፍ ጥያቄ ይላኩ',
    support_ticket_submitted: 'የድጋፍ ጥያቄዎ በተሳካ ሁኔታ ተልኳል!',
    platform_disclaimer: 'የመድረኩ ማብራሪያና ኃላፊነት',
    user_resp_transactions: '1. የተጠቃሚዎች የግብይት ኃላፊነት',
    no_auto_guarantee: '2. ዋስትና ያለመኖር ማብራሪያ',
    age_and_capacity: '3. የህግ አቅም እና እድሜ',
    limitation_liability: '4. የኃላፊነት ወሰን',
    
    // Dynamic Filter Keys
    filters: 'ማጣሪያዎች',
    reset_filters: 'ዳግም አስጀምር',
    status: 'ሁኔታ',
    property_type: 'የቤት አይነት',
    bedrooms_filter: 'ኝታ ክፍሎች',
    bathrooms_filter: 'መታጠቢያ ቤቶች',
    max_price: 'ከፍተኛ ዋጋ',
    location_placeholder: 'ከተማ፣ አካባቢ ወይም ፖስታ ሳጥን...',
    min_year_filter: 'የተመረተበት አመት',
    transmission: 'ማስተላለፊያ (ትራንስሚሽን)',
    fuel_type: 'የነዳጅ አይነት',
    job_type: 'የስራ አይነት',
    work_model: 'የስራ ሁኔታ',
    min_salary: 'ዝቅተኛ ደመወዝ',
    pricing_model: 'የዋጋ አከፋፈል ሞዴል',
    min_experience: 'ዝቅተኛ ልምድ',
    availability: 'ተገኝነት',
    min_rating: 'ዝቅተኛ ደረጃ',
    category: 'ዘርፍ',
    store_type_filter: 'የሱቅ አይነት',
    price_range: 'የዋጋ ክልል',
    
    // Filter Options
    all: 'ሁሉም',
    res_homes: 'የመኖሪያ ቤቶች',
    rent_apts: 'የኪራይ አፓርታማዎች',
    comm_re: 'የንግድ ሪል እስቴት',
    land_lots: 'ባዶ መሬቶች',
    house: 'ቤት',
    apartment: 'አፓርታማ',
    penthouse: 'ፔንትሃውስ',
    office: 'ቢሮ',
    land: 'መሬት',
    automatic: 'አውቶማቲክ',
    manual: 'ማኑዋል',
    gasoline: 'ቤንዚን',
    diesel: 'ናፍጣ',
    electric: 'ኤሌክትሪክ',
    hybrid: 'ሀይብሪድ',
    full_time: 'ሙሉ ጊዜ',
    part_time: 'ትርፍ ጊዜ',
    contract: 'ኮንትራት',
    freelance: 'ፍሪላንስ',
    internship: 'ልምምድ',
    onsite: 'በቦታው የሚሰራ',
    remote: 'ከቤት የሚሰራ',
    hourly_rate: 'በሰዓት',
    fixed_price: 'በአንድ ፕሮጀክት ቋሚ ዋጋ',
    contact_quote: 'ለዋጋው ያነጋግሩ',
    avail_today: 'ዛሬ የሚገኝ',
    avail_tomorrow: 'ነገ የሚገኝ',
    avail_this_week: 'በዚህ ሳምንት የሚገኝ',
    any_rating: 'ማንኛውም ደረጃ',
    all_stores: 'ሁሉም ሱቆች',
    verified_partner: 'የተረጋገጠ አጋር',
    independent_maker: 'ግለሰብ አምራች',
  }
};

export const AppProvider = ({ children }) => {
  const [theme, setTheme] = useState('light');
  const [language, setLanguage] = useState(localStorage.getItem('language') || 'en');
  const [activeStoreType, setActiveStoreType] = useState('');

  useEffect(() => {
    localStorage.setItem('theme', 'light');
    document.body.classList.add('light-theme');
  }, []);

  useEffect(() => {
    localStorage.setItem('language', language);
  }, [language]);

  const toggleTheme = () => {
    // Theme is always light
  };

  const toggleLanguage = () => {
    setLanguage(prev => prev === 'en' ? 'am' : 'en');
  };

  const t = (key) => {
    return translations[language][key] || key;
  };

  return (
    <AppContext.Provider value={{ theme, language, toggleTheme, toggleLanguage, t, activeStoreType, setActiveStoreType }}>
      {children}
    </AppContext.Provider>
  );
};

export const useApp = () => useContext(AppContext);
